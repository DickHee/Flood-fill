...
strides = 2*full_image.strides
view_shape = tuple(np.subtract(full_image.shape, sub_shape) + 1) + sub_shape
sub_matrices = strd(full_image,view_shape,strides,writeable=False)
...
def floodfill(sub_matrix,center_row,center_col,cluster_id):
    ...
    global full_image
    if (sub_matrix[1,1] == 1): # Part of cluster and not filled
